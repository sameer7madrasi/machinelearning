...
class RegressionModel(object):
    ...
    def train(self, dataset):
        ...
        while True:
            ...
            """lossBoy = self.get_loss(nn.Constant(dataset.x), nn.Constant(dataset.y))
            if nn.as_scalar(lossBoy) < 0.02:
                return"""
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        first_lin = nn.Linear(x, self.w1)
        first_lay = nn.AddBias(first_lin, self.b1)
        non_lin_one = nn.ReLU(first_lay)
        second_lin = nn.Linear(non_lin_one, self.w2)
        second_lay = nn.AddBias(second_lin, self.b2)
        """non_lin_two = nn.ReLU(second_lay)
        third_lin = nn.Linear(non_lin_two, self.w3)
        third_lay = nn.AddBias(third_lin, self.b3)"""
        return second_lay
